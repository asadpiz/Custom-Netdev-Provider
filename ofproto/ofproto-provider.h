...
struct oftable { 
    ...
    struct xf_tree cls_tree;    /* Tree structure for rules. */
...
struct rule {
    ...
    struct xf_rule xr;           /* Hypercuts rule */
...
static inline struct rule *
rule_from_xf_rule(const struct xf_rule *xf_rule)
{
    return xf_rule ? CONTAINER_OF(xf_rule, struct rule, xr) : NULL;
}
