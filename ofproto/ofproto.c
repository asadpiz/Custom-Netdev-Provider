...
/* oftable. */
static void oftable_init(struct oftable *, struct ofproto *);
...
ofproto_get_memory_usage(const struct ofproto *ofproto, struct simap *usage)
{
    ...
    n_rules = 0;
    OFPROTO_FOR_EACH_TABLE (table, ofproto) {
        n_rules += (classifier_count(&table->cls));// + table->cls_tree.num_rules);
    }
...
{
    //printf("XF: delete flow not currently supported");
                           ...
                           const struct ofp_header *request)
{
    ...
    for (i = 0; i < p->n_tables; i++) {
        ...
        //ots[i].active_count = htonl(p->tables[i].cls_tree.num_rules);
    }
         ...
         const struct ofputil_flow_mod *fm, const struct ofp_header *request)
{
    ...
    /* Checks if the default rule is being added */
    if (defflow < 3){
            defflow++;
    }
    else {
        rule->xr.n_actions = 1;
        switch (fm->ofpacts->type) {
            case OFPACT_OUTPUT:
                printf("ofpacts_len = %d\n",fm->ofpacts_len);
                    if (ofpact_get_OUTPUT(fm->ofpacts)[k].port == OFPP_NORMAL) 
                        strcpy(rule->xr.action[k], "normal");
                
                    else if (fm->ofpacts_len == 0) 
                        strcpy(rule->xr.action[k], "drop");
                    
                    else if (ofpact_get_OUTPUT(fm->ofpacts)[k].port == OFPP_FLOOD) 
                        strcpy(rule->xr.action[k], "flood");
                
                    else if (ofpact_get_OUTPUT(fm->ofpacts)[k].port != NULL ) {
                        //int j,k;
                        rule->xr.n_actions = 0;
                        for(j=0,k=0; j<fm->ofpacts_len; j += 8,k++) {
                            strcpy(rule->xr.action[k], "output");
                            rule->xr.interface[k] = ofpact_get_OUTPUT(fm->ofpacts)[k].port;
                            rule->xr.n_actions ++;
                        }
                    }
                    else 
                ...
                if (ntohl(fm->match.flow.nw_src) == 0) {
                    printf("setting src ip = 0\n");
                    rule->xr.field[0].low = 0;
                    rule->xr.field[0].high = 0xFFFFFFFF;
                }
                else {
                    mask = ceil (log(fm->match.wc.masks.nw_src)/log(2));
                    rule->xr.field[0].low = ntohl(fm->match.wc.masks.nw_src) & ntohl(fm->match.flow.nw_src);
                    rule->xr.field[0].high =  rule->xr.field[0].low + (1 << (32 - mask)) - 1;
                }

                if (ntohl(fm->match.flow.nw_dst) == 0){
                    rule->xr.field[1].low = 0;
                    rule->xr.field[1].high = 0xFFFFFFFF;
                }
                else {
                    mask = ceil (log(fm->match.wc.masks.nw_dst)/log(2));
                    rule->xr.field[1].low = ntohl(fm->match.wc.masks.nw_dst) & ntohl(fm->match.flow.nw_dst);
                    rule->xr.field[1].high =  rule->xr.field[1].low + (1 << (32 - mask)) - 1;
                }

                if (ntohl(fm->match.flow.tp_src) == 0){
                    rule->xr.field[2].low = 0;
                    rule->xr.field[2].high = 0xFFFF;
                }
                else {
                    rule->xr.field[2].low = ntohs(fm->match.flow.tp_src);
                    rule->xr.field[2].high = ntohs(fm->match.flow.tp_src);
                }

                if (ntohl(fm->match.flow.tp_dst) == 0){
                    rule->xr.field[3].low = 0;
                    rule->xr.field[3].high = 0xFFFF;
                }
                else {
                    rule->xr.field[3].low = ntohs(fm->match.flow.tp_dst);
                    rule->xr.field[3].high = ntohs(fm->match.flow.tp_dst);
                }

                if (ntohl(fm->match.flow.nw_proto) == 0){
                    rule->xr.field[4].low = 0;
                    rule->xr.field[4].high = 0xFF;
                }
                else {
                    rule->xr.field[4].low = fm->match.flow.nw_proto;
                    rule->xr.field[4].high = fm->match.flow.nw_proto;
                }

                rule->xr.in_port = fm->match.flow.in_port & fm->match.wc.masks.in_port;
                break;
                /* Action is not Supported by Nova 1 */
                default:
                    return OFPERR_OFPFMFC_BAD_COMMAND;
            }
            

            //printf("rule xr @ = %u\n",(unsigned int)&rule->xr);

           //if(classifier_count(&table->cls) > 10) {
                /* Insert rule into the VTT */
                cnode = xf_update_rules(&(ofproto->tables[0].cls_tree), &rule->xr);
                /* Searching if the rule belongs to a Node already in the MTT */
                if (cnode != 0)
                {
                    /* If Node is already in MTT, Replace it with updated Node */
                    if (xf_mtt_read(cnode)) {
                        //printf ("Node already in MTT, replacing with updated node\n");
                        xf_mtt_delete(cnode);
                        xf_mtt_insert(&(ofproto->tables[0].cls_tree), cnode);
                     }
                     /* Node not in MTT, Do Normal Operation 
                     else {
                         printf ("Node not in MTT\n");
                     } */
                }
            //}
        }
...
/*
    printf("rule @ : %u\n",(unsigned int)rule);
    printf("rule table_id = %d\n", rule->table_id);
    printf("rule ofpacts_len = %d\n", rule->ofpacts_len);
*/    
...
/*
    if(classifier_count(&table->cls) == 15) {
        xf_tree_build(&ofproto->tables[0].cls_tree);
    }
*/
    return 0;
}
...
ofproto_evict(struct ofproto *ofproto)
{
    ...
    OFPROTO_FOR_EACH_TABLE (table, ofproto) {
        ...
        //while (table->cls_tree.num_rules > table->max_flows
               && table->eviction_fields) {
            struct rule *rule;
...
/* Initializes 'table'. */
static void
oftable_init(struct oftable *table, struct ofproto *ofp)
{
    static int tree_init = 0;
    memset(table, 0, sizeof *table);
    classifier_init(&table->cls);
    if (tree_init == 0) {
        table->max_flows = UINT_MAX;
        xf_tree_init(&table->cls_tree, ofp);
        tree_init = 1;
        // printf("tree_init done..........\n");
        // printf("tree has %d rules\n", table->cls_tree.num_rules);
        //xf_tree_build(&table->cls_tree);
        //printf("tree_build done..........\n");
        xf_mtt_init();
        // printf("MTT init done\n");
        fflush(stdout);
...
oftable_destroy(struct oftable *table)
{
	...
	ovs_assert( table->cls_tree.num_rules == 0 );
    oftable_disable_eviction(table);
    classifier_destroy(&table->cls);
	xf_tree_destroy(&table->cls_tree);
    free(table->name);
}
...
{
    //printf("XF: remove rule not currently supported");
...
oftable_replace_rule(struct rule *rule)
{
    ...
    //victim = rule;
    eviction_group_add_rule(rule);
    //printf("rules in %d classifier = %d\n", rule->table_id, table->cls.n_rules);
...
enum ofperr
xf_ofproto_add_flow(struct xf_rule* xf_rule, struct ofproto *ofproto,
                 const struct match *match,  unsigned int priority,
                 const struct ofpact *ofpacts, size_t ofpacts_len)
{
    struct oftable *table;
    ...
    struct ofputil_flow_mod fm;
    //struct cls_rule *a;
    struct ofconn *ofconn = NULL;
    struct ofp_header *request = NULL;
    //const struct rule *add_rule;
    int error = 0;
    //int i;
    printf("starting xf_ofproto_add_flow\n");
    //rule_from_xf_rule here???
    rule_from_cls_rule(classifier_find_match_exactly(&ofproto->tables[0].cls, match, priority));

//    if (!add_rule || !ofpacts_equal(add_rule->ofpacts, add_rule->ofpacts_len,
//                                    ofpacts, ofpacts_len)) {

        printf("setting fm parameters\n");
        memset(&fm, 0, sizeof fm);
        fm.match = *match;
        fm.priority = priority;
        fm.buffer_id = UINT32_MAX;
        ...
        error = check_table_id(ofproto, fm.table_id);
        //printf("inserting in table_id: %d\n", fm.table_id);
        if (error) {
            return error;
        }
        // Pick table
        printf("table_id = %u\n", (unsigned int)fm.table_id);
        if (fm.table_id == 0xff) {
            uint8_t table_id;
            if (ofproto->ofproto_class->rule_choose_table) {
                error = ofproto->ofproto_class->rule_choose_table(ofproto,
                                                                  &fm.match,
                                                                  &table_id);
                printf("rule_choose_table done, error = %u", error);
                if (error) {
                    return error;
                }
                ovs_assert(table_id < ofproto->n_tables);
        ...
        printf("after fm.table_id iffff\n");

        table = &ofproto->tables[0]; // for now, always go to table 0
        //table->max_flows = 10000;
        if (table->flags & OFTABLE_READONLY) {
            return OFPERR_OFPBRC_EPERM;
        }
        ...
        memcpy(&rule->xr, xf_rule, sizeof(struct xf_rule));
        //xf_rule->oftable_rule = &rule;
        //printf("here\n");
        // FIXME: Implement OFPFF12_RESET_COUNTS
        rule->ofproto = ofproto;
        //rule->ofproto->ofproto_class = &ofproto_dpif_class; //XF-qasim
        rule->pending = NULL;
        rule->flow_cookie = fm.new_cookie;
        rule->created = rule->modified = rule->used = time_msec();
        rule->idle_timeout = fm.idle_timeout;
        rule->hard_timeout = fm.hard_timeout;
        rule->table_id = 0; //table - ofproto->tables;
        //printf("rule table id = %u\n", rule->table_id);
        //printf("rule 2 table id = %u\n", xf_rule->oftable_rule->table_id);
        ...
        rule->ofpacts = xmemdup(fm.ofpacts, fm.ofpacts_len);
        rule->ofpacts_len = fm.ofpacts_len;
        //FIXME: Storing actions in xf_rule too for now
        xf_rule->ofpacts = xmemdup(fm.ofpacts, fm.ofpacts_len);
        xf_rule->ofpacts_len = fm.ofpacts_len;
        //printf("storing rule, with output port = %d \n",
        //           ofpact_get_OUTPUT(xf_rule->ofpacts)->port);

        printf("rule 1 = %d\n",(unsigned int)rule);
        printf("rule table_id = %d\n", rule->table_id);
        printf("rule ofpacts_len = %u\n", rule->ofpacts_len);
        rule->evictable = true;
        rule->eviction_group = NULL;
        list_init(&rule->expirable);
        rule->monitor_flags = 0;
        ...
        //rule->xr = *xf_rule;
        //memcpy(&rule->xr, xf_rule, sizeof(struct xf_rule));
        //printf("original action = %s\n", xf_rule->action);
        //printf("actions length = %d\n", rule->ofpacts_len);

        // Insert new rule.
        victim = oftable_replace_rule(rule);
        xf_rule->oftable_rule = rule;
        printf("rule 2 = %d\n",(unsigned int)rule);
        printf("rule table_id = %d\n", (unsigned int)rule->table_id);
        printf("rule ofpacts_len = %u\n", rule->ofpacts_len);
        printf("@xf_rule in tree = %u\n",(unsigned int)xf_rule);
        //printf("and now here\n");
        if (!rule) printf("rule is now NULL");
        if (!rule->ofproto->ofproto_class) printf("ofproto_class is NULL");
        if (victim && !rule_is_modifiable(victim)) {
            error = OFPERR_OFPBRC_EPERM;
        } else if (victim && victim->pending) {
            error = OFPROTO_POSTPONE;
            ...
            struct ofoperation *op;
            struct rule *evict;
            printf("classifier_count = %d\n",classifier_count(&table->cls));
            printf("table max_flows = %u\n", table->max_flows);
            if (classifier_count(&table->cls) > table->max_flows) {
                bool was_evictable;
                was_evictable = rule->evictable;
                rule->evictable = false;
                ...
                if (!evict) {
                    error = OFPERR_OFPFMFC_TABLE_FULL;
                    printf("not evict error\n");
                    goto exit;
                } else if (evict->pending) {
                    printf("evict pending error\n");
                    error = OFPROTO_POSTPONE;
                    goto exit;
                }
            } else {
            ...
            error = ofproto->ofproto_class->rule_construct(rule);
            ovs_assert(rule->ofproto->ofproto_class == &ofproto_dpif_class);
            printf("assertion passed in add_flow \n");
            printf("rule ofproto_class = %u\n", (unsigned int)rule->ofproto->ofproto_class);
            printf("ofproto_dpif_class = %u\n", (unsigned int)&ofproto_dpif_class);

            if (error) {
                printf("rule_construct error\n");
                op->group->n_running--;
                ofoperation_destroy(rule->pending);
            } else if (evict) {
                printf("evict is true\n");
                delete_flow__(evict, group);
            }
            printf("going to ofofgroup_submit\n");
            ofopgroup_submit(group);
        }
//    }
