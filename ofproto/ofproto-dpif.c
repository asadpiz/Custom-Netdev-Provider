...
/*    
    ...
    printf("rule @: %u \n", (unsigned int)rule);
    printf("rule.up @: %u \n", (unsigned int)&rule->up);
    printf("rule->up.ofproto @: %u \n", (unsigned int)rule->up.ofproto);
    printf("rule->up.table_id  = %u \n", rule->up.table_id);
    printf("rule->up.table_id @  = %u \n", &(rule->up.table_id));
    printf("rule.ofproto_class = %u \n", rule->up.ofproto->ofproto_class );
*/
    //printf("assertion time\n");
    ovs_assert(rule->up.ofproto->ofproto_class == &ofproto_dpif_class);
    //printf("assertion ok\n");

    ofproto = ofproto_dpif_cast(rule->up.ofproto);
    now = time_msec();
                 ...
                 size_t *n_ops)
{
    ...
    fm_aclValue *value = malloc(sizeof(fm_aclValue));
    fm_aclAction action;
...
//        now = time_msec();
//    }
//    printf("going to: handle_flow_miss_with_facet\n");
                    ...
                    size_t n_upcalls)
{
    ...
    //printf("assertion succesfull & n_ops = %d\n",n_ops);
    /* Execute batch. */
    for (i = 0; i < n_ops; i++)
    {
        dpif_ops[i] = &flow_miss_ops[i].dpif_op;
    }
    // dont need this now, right? 
    dpif_operate(backer->dpif, dpif_ops, n_ops);

    //printf("dpif operate done\n");
    /* Free memory. */
    for (i = 0; i < n_ops; i++) {
        free(flow_miss_ops[i].garbage);
    }
    hmap_destroy(&todo);
    //printf("handle_flow_miss_done\n");
}
                   ...
                   uint8_t table_id)
{
    ...
    unsigned int hdr[MAXDIMS+1];
    struct xf_rule *xf_rule;
    struct xf_tree *cls_tree;
    struct xf_lookup_match match; 
    int i;
    struct rule *x = (struct rule *)malloc(sizeof(struct rule));
    if (table_id >= N_TABLES) {
        return NULL;
    }
    hdr[0] = htonl(flow->nw_src);
    hdr[1] = htonl(flow->nw_dst);
    hdr[2] = htons(flow->tp_src);
    hdr[3] = htons(flow->tp_dst);
    hdr[4] = flow->nw_proto;
    hdr[5] = flow->in_port;
    cls = &ofproto->up.tables[table_id].cls;
    /* we always want the tree lookup to be in tables[0] */
    cls_tree = &ofproto->up.tables[0].cls_tree;
    //printf("Tree has %d rules\n",cls_tree->num_rules);
#if DEBUG >= 2
    printf("Header[0] = %u Header[1] = %u \n",hdr[0],hdr[1]);
    printf("Header[2] = %u Header[3] = %u \n",hdr[2],hdr[3]);
    printf("Header[4] = %u Header[5] = %u \n",hdr[4],hdr[5]);
    fflush(stdout);
#endif
        ...
        && ofproto->up.frag_handling == OFPC_FRAG_NORMAL)
    {
        ...
        printf("cls_rule @: %u\n", (unsigned int)cls_rule);
        xf_rule = xf_tree_lookup(cls_tree, hdr, &match);
#if DEBUG >= 2
    ...
    {
#if DEBUG >= 2
        printf("match found in rule: %d node: %d\n", match.rule_id,match.node_id);
#endif
        xf_mtt_insert(cls_tree, match.node_id);
              
    }        
    
    if (xf_rule != NULL) {
       return xf_rule->rule_ptr;
    }
    else {
                  ...
                  struct ofpbuf *packet)
{
    ...
    ctx.resubmit_stats = &stats;
    //printf("1 translating output with port = %d \n", 
    //        ofpact_get_OUTPUT(rule->up.ofpacts)->port);
    xlate_actions(&ctx, rule->up.ofpacts, rule->up.ofpacts_len, &odp_actions);
                 ...
                 struct action_xlate_ctx *ctx)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        //printf("inside do_xlate_actions 4\n");
        //printf("actions length = %d\n",ofpacts_len);
        //printf("action type = %d\n",a->type);
        switch (a->type) {
        case OFPACT_OUTPUT:
            //printf("3 translating output with port = %d, max_len = %d\n",
            //       ofpact_get_OUTPUT(a)->port,ofpact_get_OUTPUT(a)->max_len);
              ...
              struct ofpbuf *odp_actions)
{
    ...
    //printf("inside xlate_actions 3\n");
    
    if (ctx->flow.nw_frag & FLOW_NW_FRAG_ANY) {
        //printf("inside xlate_actions 4\n");
    ...
    //printf("inside xlate_actions 5\n");

    in_port = get_ofp_port(ctx->ofproto, ctx->flow.in_port);
    special = process_special(ctx->ofproto, &ctx->flow, in_port, ctx->packet);
    if (special) {
        //printf("inside xlate_actions 6\n");
        ctx->slow |= special;
    } else {
        //printf("inside xlate_actions 7\n");
        ...
        //printf("inside xlate_actions 8\n");

        if (tunnel_ecn_ok(ctx) && (!in_port || may_receive(in_port, ctx))) {
            //printf("2 translating output with port = %d \n", 
            //       ofpact_get_OUTPUT(ofpacts)->port);
        ...
        if (ctx->max_resubmit_trigger && !ctx->resubmit_hook) {
            if (!hit_resubmit_limit) {
                 ...
                 * now on. */
                //printf("inside xlate_actions 10\n");
                hit_resubmit_limit = true;
            } else if (!VLOG_DROP_ERR(&trace_rl)) {
                struct ds ds = DS_EMPTY_INITIALIZER;
                //printf("inside xlate_actions 11\n");
        ...
        //printf("inside xlate_actions 14\n");
