...
#include "ofproto/xf_hypercuts.h"
#include "poll-loop.h"
#include "sha1.h"
#include "shash.h"
#include "smap.h"
...
bridge_update_ofprotos(void)
{
    ...
    HMAP_FOR_EACH_SAFE (br, next, node, &all_bridges) {
        ...
        //printf("ofproto %s created with %d tables\n", br->ofproto->name, br->ofproto->n_tables);
        if (error) {
...
void
bridge_run(struct xf_rule* rule, int num_rules)
{
...
#if CONTROLLER == 0
        if (br && br_init == 0) {
            if ( access("/dev/shm/usr/var/run/openvswitch/br0.mgmt", F_OK) == 0) {
            //if ((fpr = fopen("/dev/shm/usr/var/run/openvswitch/br0.mgmt", "r") != NULL)) {
                // printf("num_rules in tree = %d\n", br->ofproto->tables[0].cls_tree.num_rules);
                br_init = 1;
                for (i = 0; i < num_rules ; i++) {
#if DEBUG >= 2
                    printf("adding rule num %d\n with first action = %s\n",i,rule[i].action[0]);
#endif                 
                    xf_add_rule(br->ofproto,&rule[i]);
                }
#if DEBUG >= 1
                printf("starting tree_build\n");
#endif                 
                xf_tree_build(&br->ofproto->tables[0].cls_tree);
#if DEBUG >= 1
    ...
    if (time_msec() >= iface_stats_timer) {
        if (cfg) {
            ...
            refresh_controller_status(rule,num_rules);
            ovsdb_idl_txn_commit(txn);
...
int xf_load_rules(char* rules, struct xf_rule* rule) {
    ...
    char* inter = (char*)malloc(2*sizeof(char));
    FILE *fpr;
    fpr = fopen("rules", "r");
    ...
    while (fgets(s, 300, fpr) != NULL) {
        k++;
    }

    rewind(fpr);
    //rule = (struct xf_rule*)calloc(k, sizeof(struct xf_rule));

    printf("middle of load_rules, %d rules read\n", k);
    while (1) {
        if (i == k) { /* all rules loaded */
            break;
        }
        //rule[i].interface = 0;
        rule[i].n_actions = 0;
        j = fscanf(fpr, "@%u.%u.%u.%u/%u\t%u.%u.%u.%u/%u\t%u : %u\t%u : %u\t%x/%x\t%u\t%s",
                   &sip1, &sip2, &sip3, &sip4, &siplen, &dip1, &dip2, &dip3, &dip4, &diplen,
                   &rule[i].field[2].low, &rule[i].field[2].high, &rule[i].field[3].low,
                   &rule[i].field[3].high, &proto, &protomask, &rule[i].in_port,&rule[i].action[0]);

        if (j < 18) {
            break;
        }
        
        if(!strcmp(rule[i].action[0],"drop")){
           rule[i].n_actions = 1;
           rule[i].interface[0] = 0;
           fscanf(fpr,"\t\n");
        }
        else if(!strcmp(rule[i].action[0],"flood")) {
           rule[i].n_actions = 1;
           fscanf(fpr,"\t\n");
        }
        else if(!strcmp(rule[i].action[0],"normal")) {
           rule[i].n_actions = 1;
           fscanf(fpr,"\t\n");
        }
        else if(!strcmp(rule[i].action[0],"output")) {
            j =fscanf(fpr," : %u\t",&rule[i].interface[0]);
            if (j < 1) {
               printf("output interface not valid\n");
               break;
            }
        }
        ...
        j = fscanf(fpr,"%s\n",&rule[i].action[1]);
        if (j==1) {
            if(!strcmp(rule[i].action[1],"mirror")) {
                rule[i].n_actions = 2;
                printf("2nd action = %s\n", rule[i].action[1]);
            }
            else {
                fseek(fpr,- (strlen(rule[i].action[1])) -1, SEEK_CUR);
                rule[i].n_actions = 1;
                strcpy(rule[i].action[1],"NULL");
            }
        }
        else {
           rule[i].n_actions = 1;
    ...
    free(tmp_str);
    free(inter);
    /*printf("finished load_rules\n"); */
    return i;  /* pointer to rules loaded */
}
