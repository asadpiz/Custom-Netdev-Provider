...
/* Global integer to indicate if initialization done*/
//int intinit = 0;
//bool recvFlag = TRUE;

/* Semaphore to indicate switch state*/
//fm_semaphore seqSem;

/* Global buffer for storing received packet */
...
/*
void eventHandler(fm_int event, fm_int sw, void* ptr)
{
    fm_eventPktRecv* pktEvent = 0;
    fm_buffer* recvPkt = 0;
    ...
    case FM_EVENT_PKT_RECV:
        ...
        printf("packet received by Intel\n");
...
/*
    if (!intinit)
    {
        ...
        for (cpi = 1 ; cpi < swInfo.numCardPorts ; cpi++)
        {
            ...
            if ( ( err = fmSetVlanPortState(sw, 1, port, FM_STP_STATE_FORWARDING) ) != FM_OK )
            {
        ...
        acl = 0;
        if ( ( err = fmCreateACL(sw, acl) ) != FM_OK )
        {
            cleanup("fmCreateACL", err);
        }

        printf("ACL 0 created\n");
        ...
        if ( ( err = fmAddACLRule(sw, acl, rule, cond, &value, \
                                  action, 0) ) != FM_OK )
        {
            cleanup("fmAddACLRule", err);
        }
        printf("Rule 0 added to ACL 0\n");
        ...
        printf("added ACL to ports\n");

        if ( (err = fmCompileACL(sw, status, 100, 0) ) != FM_OK )
        {
...
netdev_linux_recv(struct netdev *netdev_, void *data, size_t size)
{
    ssize_t retval = 0;
    struct netdev_linux *netdev = netdev_linux_cast(netdev_);
    //if(!strcmp(netdev_->netdev_dev->name,"br0"))
    //{
        if (netdev->fd < 0) {
...
/*
    if(strcmp(netdev_->netdev_dev->name, "br0"))
    {
        printf("inside my netdev_recv\n");
        if(soft_q[1].n_pkts > 0 || soft_q[3].n_pkts > 0)
        {
            if(recvFlag && soft_q[1].n_pkts > 0)
            {
                retval = sq_pop(1, &soft_q, data, size);
                printf("Packet Poped from port 1 Queue\n");
                recvFlag = FALSE;
            }
            else if(!recvFlag && soft_q[3].n_pkts > 0)
            {
                printf("Packet Poped from port 3 Queue\n");
                retval = sq_pop(3, &soft_q, data, size);
                recvFlag = TRUE;
            }
