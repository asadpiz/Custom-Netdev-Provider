                             ...
                             const struct cls_rule *target)
{
    ...
    if (!table) {
        //printf("in find_rule_exactly, table not found\n");
        return NULL;
    }
    ...
    head = find_equal(table, &target->match.flow,
                      miniflow_hash_in_minimask(&target->match.flow,
                                                &target->match.mask, 0));
    //printf("in find_rule_exactly, table found\n");
                              ...
                              unsigned int priority)
{
    ...
    struct cls_rule cr;
    //printf("in find_match_exactly()\n");
    cls_rule_init(&cr, target, priority);
    //printf("rule initialized\n");
    retval = classifier_find_rule_exactly(cls, &cr);
    //printf("after find_rule_exactly()\n");
